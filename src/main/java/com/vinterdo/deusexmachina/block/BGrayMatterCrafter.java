...
public class BGrayMatterCrafter extends BTileEntityDEM
{
	public BGrayMatterCrafter()
	{
		super();
		this.setBlockName("grayMatterCrafter");
	...
	{
		return new TEGrayMatterCrafter();
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ)
	{
		if(!world.isRemote)
		{
			TEGrayMatterCrafter te = (TEGrayMatterCrafter)world.getTileEntity(x, y, z);
			if(te.isFormed()) 
			{
				player.openGui(DeusExMachina.instance, GuiHandler.GuiIDs.GRAY_MATTER_CRAFTER.ordinal(), world, te.getMaster().xCoord, te.getMaster().yCoord, te.getMaster().zCoord);
				return true;
			}
			else
			{
	...
	{
		TEGrayMatterCrafterMaster te = (TEGrayMatterCrafterMaster) ((TEGrayMatterCrafter)world.getTileEntity(x, y, z)).getMaster();
		if(te!= null) 
			te.removeMember((TEGrayMatterCrafter)world.getTileEntity(x, y, z));
	...
	{
			TEGrayMatterCrafter teMe = (TEGrayMatterCrafter) world.getTileEntity(x - ForgeDirection.getOrientation(side).offsetX, y - ForgeDirection.getOrientation(side).offsetY, z - ForgeDirection.getOrientation(side).offsetZ);
