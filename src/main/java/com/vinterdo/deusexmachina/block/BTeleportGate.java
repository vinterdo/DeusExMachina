...
public class BTeleportGate extends BTileEntityDEM
{
	public BTeleportGate()
	{
		super();
		this.setBlockName("teleportGate");
			...
			float hitY, float hitZ)
	{
		if (!world.isRemote)
		{
			TETeleportGate te = (TETeleportGate) world.getTileEntity(x, y, z);
			if (te.isFormed())
			{
				player.openGui(DeusExMachina.instance, GuiHandler.GuiIDs.TELEPORT_GATE.ordinal(), world,
	...
	{
		TETeleportGateMaster te = (TETeleportGateMaster) ((TETeleportGate) world.getTileEntity(x, y, z))
				.getMaster();
		if (te != null)
			te.removeMember((TETeleportGate) world.getTileEntity(x, y, z));
	...
	{
		TETeleportGate teMe = (TETeleportGate) world.getTileEntity(x - ForgeDirection.getOrientation(side).offsetX,
